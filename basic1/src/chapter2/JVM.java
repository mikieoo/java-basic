...
// Heap Area (힙 영역)
// : 모든 곳에서 공유 가능한 영역, 인스턴스 혹은 배열과 같은 new 연산자를 통해 생성된

// Stack Area (스택 영역)
// : 각 스레드 (실행중인 프로세스) 별로 독립적으로 존재하는 영역, 메서드 호출과 로컬 변수에 대한 정보가 저장됨
// 각 코드블록마다 하나의 스택이 생성됨
...
public class JVM {
	...
	public static void main(String[] args) {
		...
		ExampleClass2.staticVariable = 55;
		
		instance1.instanceMethod();
		
		method1(localVariable1);
		
		System.out.println(localVariable1); // 30 기본형 매개변수
		
		method2(instance1);
		
		System.out.println(instance1.instanceVariable); // 99999 참조형 매개변수
	}
